//Also, train an HMM with N=2 hidden states on English text, then use the model in a generative mode to generate 100 characters of fake English text. 
//Repeat with N=4 and N=8 hidden states. 
//Do more hidden states produce more realistic fake English text?
...
// Using the scaling coefficients c_t, compute log P(O|lambda) = -sum_t log c_t
double compute_log_prob(const vector<double>& scaler) {
    double logProb = 0.0;
    for (double c : scaler) {
        if (c <= 0.0 || isnan(c) || sinf(c)) {
...
//helper function
// Sample an index from a categorical distribution (unnormalized weights OK)
int sample_cat(const std::vector<double>& w, mt19937& rng) {
    discrete_distribution<int> dist(w.begin(), w.end());
    return dist(rng);
}

// Generate T characters from the trained HMM (states -> {a..z, space})
std::string generate_text(int T,
                          const vector<double>& pi,                 // size N
                          const vector<std::vector<double>>& A,     // N x N
                          const vector<std::vector<double>>& B) {   // N x 27
    // observation alphabet: 'a'..'z',' '
    vector<char> obs2char;
    for (char c='a'; c<='z'; ++c) obs2char.push_back(c);
    obs2char.push_back(' '); // index 26

    mt19937 rng(12345); // fixed seed for reproducibility

    string out; out.reserve(T);
    int s = sample_cat(pi, rng);                   // initial hidden state
    for (int t = 0; t < T; ++t) {
        int k = sample_cat(B[s], rng);             // emit observation (character index)
        out.push_back(obs2char[k]);                // append character
        s = sample_cat(A[s], rng);                 // transition to next state
    }
    return out;
}
...
int main(int argc, char *argv[])
{
    ...
    int M = 27;            // number of all possible observations ( a to z and a space )
    ...
    // printHMMParameters(pi, A, B);
    string sample = generate_text(100, pi, A, B);
    cout << "\n=== GENERATED TEXT (N=" << N << ", 100 chars) ===\n";
    cout << sample << "\n";
    cout << "===============================================\n";
    return 0;
}
